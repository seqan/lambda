 ...
 * \brief Provides views::async_input_buffer.
 */
...
/*!\brief The type returned by views::async_input_buffer.
...
private:
    ...
    struct state
    {
        ...
        bio::io::contrib::fixed_buffer_queue<std::ranges::range_value_t<urng_t>> buffer;
    ...
    //!\brief The iterator of the detail::async_input_buffer_view.
    class iterator;
    ...
    async_input_buffer_view(urng_t _urng, size_t const buffer_size)
    {
        ...
        state_ptr = std::shared_ptr<state>(
          new state{std::move(_urng),
                    bio::io::contrib::fixed_buffer_queue<std::ranges::range_value_t<urng_t>>{buffer_size},
        ...
        auto runner = [&state = *state_ptr]()
        {
            for (auto && val : state.urange)
                if (state.buffer.wait_push(std::move(val)) == bio::io::contrib::queue_op_status::closed)
                    break;
...
class async_input_buffer_view<urng_t>::iterator
{
    ...
    bio::io::contrib::fixed_buffer_queue<std::ranges::range_value_t<urng_t>> * buffer_ptr = nullptr;
...
public:
    ...
    */
    ...
    //!\brief Pointer type.
    using pointer           = bio::ranges::detail::iter_pointer_t<urng_iterator_type>;
    ...
    //!\brief Constructing from the underlying async_input_buffer_view.
    iterator(bio::io::contrib::fixed_buffer_queue<std::ranges::range_value_t<urng_t>> & buffer) noexcept :
      buffer_ptr{&buffer}
    {
    ...
    iterator & operator++() noexcept
    {
        ...
        if (buffer_ptr->wait_pop(cached_value) == bio::io::contrib::queue_op_status::closed)
            at_end = true;
...
/*!\name Deduction guide.
 * \relates detail::async_input_buffer_view
 * \{
 */
...
//!\brief Definition of the range adaptor object type for views::async_input_buffer.
struct async_input_buffer_fn
{
    ...
    {
        return bio::ranges::detail::adaptor_from_functor{*this, buffer_size};
    }
    ...
    constexpr auto operator()(urng_t && urange, size_t const buffer_size) const
    {
        ...
        return async_input_buffer_view{std::forward<urng_t>(urange), buffer_size};
    }
};
 ...
 */
inline constexpr auto async_input_buffer = async_input_buffer_fn{};
} // namespace views
