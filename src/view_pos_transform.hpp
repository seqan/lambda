...
public:
    ...
    view_pos_transform(urng_t _urange, pos_transform_t _pos_transform, size_transform_t _size_transform) :
      urange{std::move(_urange)}, pos_transform{std::move(_pos_transform)}, size_transform{std::move(_size_transform)}
    {}
    ...
    const_iterator begin() const noexcept requires seqan3::const_iterable_range<urng_t> { return {*this, 0}; }

    iterator end() noexcept { return {*this, size()}; }

    const_iterator end() const noexcept requires seqan3::const_iterable_range<urng_t> { return {*this, size()}; }

    // Returns the number of elements in the view.
    size_type size() noexcept { return (*size_transform)(*urange); }

    size_type size() const noexcept requires seqan3::const_iterable_range<urng_t> { return (*size_transform)(*urange); }
    ...
    const_reference operator[](size_type const n) const requires seqan3::const_iterable_range<urng_t>
    {
        assert(n < size());
        return (*pos_transform)(*urange, n);
    }
...
view_pos_transform(urng_t &&, pos_transform_t, size_transform_t)
  -> view_pos_transform<std::views::all_t<urng_t>, pos_transform_t, size_transform_t>;
...
public:
    ...
    template <std::ranges::range urng_t, typename pos_transform_t, typename size_transform_t = default_size_fn>
